...
stm:            ID ASSIGN expr SEMI                                                         #IDExprStm
                | ID LBR arg_list RBR SEMI                                                  #IDArgListStm
                | Begin statement_block End                                                 #BlockStm
                | If condition Begin statement_block End Else Begin statement_block End     #IfBlocktm
                | While condition Begin statement_block End                                 #WhileStm
                | Skip SEMI                                                                 #SkipStm
                ;

expr:	        expr binary_arith_op expr       #BinOpExpr
                |LBR expr RBR                   #BracExpr
                |ID LBR arg_list RBR            #IDExpr
                |frag                           #FragExpr
                ;
...
condition:      LOGNEG condition                                #NegCond
                | LBR condition RBR                             #BrackCond
                | expr comp_op expr                             #ExprCompOpCond
                | condition op=(OR | AND) condition             #OrAndCond
                ;
